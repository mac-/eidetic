...
describe('Eidetic Cache', function() {
	...
	describe('get()', function() {
		...
		it('should get the cached value after the original expiration when using a sliding expiration', function(done) {
			...
			setTimeout(function() {
				value = cache.get('key');
				assert.deepEqual(value, testValue, 'The value should be cached');
				setTimeout(function() {

					value = cache.get('key');
					assert.deepEqual(value, testValue, 'The value should be cached beyond it\'s original expiration');
					done();
	...
	describe('put()', function() {
		...
		it('hitting the max cache size on put should remove the least recently used', function(done) {
			...
			setTimeout(function() {
				...
				setTimeout(function() {
					...
					value = cache.get('key1');
					assert.deepEqual(value, testValue, 'the second cached value should be in the cache still');
					done();
				}, 100);
		...
		it('hitting the max cache size on put should not remove the least recently used', function(done) {
			...
			setTimeout(function() {
				...
				setTimeout(function() {
					var value = cache.get('key');
					assert.deepEqual(value, testValue, 'the first cached value should not be removed from the cache before it\'s expiration');
					value = cache.get('key1');
		...
		it('insert item with same key twice', function(done) {
			...
			var value = cache.get('key');
			assert.deepEqual(value, testValue2, 'the first cached value should be overwritten with the second');
			done();
		...
		it('update cached value without modifying it\'s expiration', function(done) {
